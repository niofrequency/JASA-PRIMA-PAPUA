...
export async function generateCourseFromSafetyCultureServer(
  ...
  for (const [idx, lesson] of (scData.lessons || []).entries()) {
    ...
    const hasRealSlideContent = !!(lesson.slides && lesson.slides.length > 0);
      ...
      lessons: [
        {
          ...
          readingMaterial: hasRealSlideContent
            ? `**SafetyCulture Reference Data:**\n- Imported directly from verified accredited K3 training slides.\n- Follow local Papuan environmental and ESDM safety standards.`
            : `**⚠️ No SafetyCulture slide content available for this lesson.**\n- The content above was generated by Gemini from the lesson title only, not from real SafetyCulture material.\n- Follow local Papuan environmental and ESDM safety standards until verified source content is available.`,
        },
      ],
      quiz:
